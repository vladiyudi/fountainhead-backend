...
exports.getProjectVotes = async (req, res) => {
    try {
        ...
        const clientVotes = await knex("clientsVotes").where({
            projectId: projectId,
        });
        res
            .status(200)
            .json({ studentVotes: studentVotes[0], clientVotes: clientVotes[0] });
    } catch (err) {
        console.log(err);
...
exports.sortByVotes = async (req, res) => {
    try {
        ...
        if (role === 'student') {
            ...
            const projects = await knex('projects').join('clientsVotes', 'projects.projectId', 'clientsVotes.projectId').orderBy(sortBy, 'desc')
            res.status(200).json(projects)
        }
    }
